...
def rand_flip_MCD(img1, img2, label1, label2, lable_class):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, img2, label1, label2, lable_class
    elif r < 0.5:
        return np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label2, axis=0).copy(), np.flip(lable_class, axis=0).copy()
    elif r < 0.75:
        return np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label2, axis=1).copy(), np.flip(lable_class, axis=1).copy()
    else:
        return img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), label1[::-1, ::-1].copy(), label2[::-1, ::-1].copy(), lable_class[::-1, ::-1].copy()

def rand_flip_MCD5(img1, img2, label1, label2, label3, label4):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
...
def rand_rot90_MCD(img1, img2, label1, label2, lable_class):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, img2, label1, label2, lable_class
    else:
        return np.rot90(img1).copy(), np.rot90(img2).copy(), np.rot90(label1).copy(), np.rot90(label2).copy(), np.rot90(lable_class).copy()

def rand_rot90_MCD5(img1, img2, label1, label2, label3, label4):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, img2, label1, label2, label3, label4
...
def rand_rot90_flip_MCD(img1, img2, label1, label2,label_class):
    img1, img2, label1, label2, label_class = rand_rot90_MCD(img1, img2, label1, label2,label_class)
    return rand_flip_MCD(img1, img2, label1, label2,label_class)

def rand_rot90_flip_MCD5(img1, img2, label1, label2, label3, label4):
